...
export default {
  async fetch(request, env) {
    ...
    async function copyMessage(fromChatId, messageId, toChatId) {
      const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/copyMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          message_id: messageId,
          chat_id: toChatId,
          caption: '💎 Join For More -> @hive_zone'
        })
      });
    ...
    async function handleCallbackQuery(update) {
      ...
      if (data.startsWith('retry_')) {
        ...
        if (!userJoined) {
          ...
          await editMessage(userId, query.message.message_id, '✅ Verification successful!');
          await copyMessage(STORAGE_CHANNEL, msgId, userId);
        }
      }
    }
